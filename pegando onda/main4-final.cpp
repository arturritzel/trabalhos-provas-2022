...
int main()
{
    ...
    cout << endl;
    cout << "manipulacao de arquivos wav" << endl;
    cout << endl;
    ...
    tempo.total = cab.Subchunk2Size/cab.ByteRate; // calcula o tempo total do arquivo wav

    cout << "iniciar a partir de qual segundo?" << endl;
    cout << "entrada: ";
    cin >> tempo.inicio; // entrada para tempo inicial

    while(tempo.inicio < 0 || tempo.inicio >= tempo.total){
        cout << "entrada invalida." << endl;
        cout << "verifique se a entrada esta em segundos positivos e menor que o tempo total do arquivo e tente novamente" << endl;
        cin >> tempo.inicio;
    }
    ...
    if(entrada == 2){ // caso estilo de corte 2
        cout << "o corte deve acabar em qual segundo?" << endl;
        cout << "entrada: ";
        cin >> tempo.fim;
        ...
        tempo.corte = tempo.fim - tempo.inicio; // calcula o tempo do corte de acordo com as informacoes
    }



        if(tempo.inicio+tempo.corte > tempo.total){ // testa se o corte ultrapassaria o tempo total do arquivo
            cerr << "erro: o corte ultrapassaria o tempo do arquivo." << endl;
            cerr << "verifique se os valores de entrada estao corretos e tente novamente" << endl;
            return ERR_INPUT;
        }

        if(tempo.corte <= 0){ // testa se o tempo do corte é menor ou igual a 0 (o audio precisa ter um tempo positivo em segundos)
            cerr << "erro: o tempo do corte deve ser maior que 0." << endl;
            return ERR_INPUT;
        }
    ...
    if(!outFile){ // testa se o arquivo nao foi aberto corretamente
        cerr << "O arquivo '" << arquivo_saida << "' nao pode ser aberto." << endl;
        return ERR_UNKNOWN;
    }
    ...
    for(int i = 0; i < tempo.inicio*cab.ByteRate; i++){ // para cada segundo de audio antes do segundo inicial do corte
        inFile.read((char *)&leitura, sizeof(char)); // le todos as amostras ate o tempo inicial (mas nao faz nada com elas)
    }

    for(int i = 0; i < tempo.corte*cab.ByteRate; i++){ // para cada segundo de audio entre o segundo inicial e o segundo final
    ...
    cout << "o programa ira encerrar." << endl;
