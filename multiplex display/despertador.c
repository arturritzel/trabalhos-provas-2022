/*
  fazer uma variavel global tipo
  horario do cronometro ja definido = 0;
  
  nos timers, fazer um if:
  
  if ja definido: funcionamento padrao do relogio
  if nao definido: funcionamento para definir
  
  da pra fazer 2 whiles tambem, um pra definir e outro pra contar/despertar
  
  quando verificar que o tempo é igual ao que eu queria, da pra desativar as interrupcoes dos timers
  
  if((minuto1 > 3 && minuto2 > 5) || hora > 4), por exemplo, se eu definir 45 minutos
  
  (isso ta dando um erro do caralho)
*/
...
int definido_despertou = 0; // 0 nao definiu horario, 1 horario definido, 2 despertou
...
int horario[7] = {0, 0, 0, 0, 0, 0, 10};
int despertar[7] = {0, 0, 0, 0, 0, 0, 10};
...
// hora, hora, indef, minuto, minuto, indef, seg, seg, indef, indef, indef
int rotacao[11] = {0, 1, 6, 2, 3, 6, 4, 5, 6, 6, 6};

void TIM1_UP_TIM10_IRQHandler(){
	if(definido_despertou == 0){

	}

	if(definido_despertou == 1){
...
int main(){
	...
	int disp_definicao = 0;

	/*
	while(disp_definicao == 0){ // definindo segundos
		if((GPIOA->IDR&=0x10)==0x10 && apertou[1] == 0) apertando[1]++; // caso pb4 esteja sendo apertado
		if((GPIOA->IDR&=0x10)!=0x10 && apertou[1] == 1) apertou[1] = 0; // caso pb4 nao esteja sendo apertado
		...
		//////////////
		if(disp_definicao == 0){
			GPIOC->ODR = ~valorDisplay[0];
			GPIOB->ODR = 0x1;
		}
		if(disp_definicao == 1){
			GPIOC->ODR = ~valorDisplay[1];
			GPIOB->ODR = 0x2;
		}
		disp_definicao++;
		if(disp_definicao > 1) disp_definicao = 0;



		if((GPIOC->IDR&=0x2000)!=0x10)	break; // confirmar
	} // */


	disp_definicao = 1;

	while(disp_definicao == 1){
		if((GPIOA->IDR&=0x01)==0x01 && apertou[3] == 0) apertando[3]++; // caso pb0 esteja sendo apertado
		if((GPIOA->IDR&=0x02)==0x02 && apertou[2] == 0) apertando[2]++; // ==== pb1 =====================
		if((GPIOA->IDR&=0x10)==0x10 && apertou[1] == 0) apertando[1]++; // ==== pb4 =====================

		if((GPIOA->IDR&=0x01)!=0x01 && apertou[3] == 1) apertou[3] = 0; // caso pb0 nao esteja sendo apertado
		if((GPIOA->IDR&=0x02)!=0x02 && apertou[2] == 1) apertou[2] = 0; // ==== pb1 =========================
		if((GPIOA->IDR&=0x10)!=0x10 && apertou[1] == 1) apertou[1] = 0; // ==== pb4 =========================
